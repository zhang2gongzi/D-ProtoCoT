...
The corrected statement reflects the actual pipeline: each backbone is processed independently, and the question-grouped $8{:}1{:}1$ split is applied to that backbone's own sampled reasoning-path pool. The split sizes therefore differ across backbones, as Table~\ref{tab:dataset-details} reports:
...
**Accuracy.** Test-time selection accuracy under the two schemes differs by $2.50$ points: $79.50\%$ (newline) vs. $82.00\%$ (`Step k:`). This gap sits within the run-to-run variance we already document for this setting: Table~\ref{tab:main-results} reports D-ProtoCoT at $82.00\%$ on GSM8K / Qwen3-8B, while the granularity ablation in Table~\ref{tab:granularity-ablation} reports $84.50\%$ for the same configuration, a $2.50$-point gap that we attribute to training stochasticity (different random initialization / epoch checkpoint). The newline-vs-`Step k:` difference falls in the same noise band. We therefore conclude that the method is robust to the choice of step delimiter.
